...
while [ $SECONDS -lt $end ]; do
  ...
  if [[ $APP_ENVIRONMENT == *"Production"* ]]; then
    ...
    curl "$APP_URL/api/invoke?url=https://www.dynatrace.com/news/" --silent --output /dev/null
    ...
    curl "$APP_URL/api/invoke?url=http://blog.dynatrace.com" -H "x-dynatrace: NA=Test.Invoke;" --silent --output /dev/null
    curl "$APP_URL/api/invoke?url=https://www.dynatrace.com/news/" -H "x-dynatrace: NA=Test.Invoke;" --silent --output /dev/null
    sleep 5;
  fi
done;
exit 0